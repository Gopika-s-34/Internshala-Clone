...
import { ArrowUpRight, Book, Calendar, Clock, DollarSign, ExternalLink, MapPin, X } from 'lucide-react';
...
const index = () => {
    ...
    //         AdditionalInfo: " role is hybrid with occasional onsite meetings.",
    //         numberOfOpening: "3",
    //     },
    //     {
    //         _id: "102",
    ...
    useEffect(() => {
        const fetchdata = async () => {
            if (!id) return;
            try {
                const res = await axios.get(`https://internshala-clone-8uer.onrender.com/api/job/${id}`);
                setjob(res.data)
            } catch (error) {
                console.log(error)
            }
